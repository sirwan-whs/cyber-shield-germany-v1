...
import { ExternalWebPanel } from '@/components/ExternalWebPanel';
import { SensorNetworkPanel } from '@/components/SensorNetworkPanel';
  ...
  private createPanels(): void {
    ...
    if (SITE_VARIANT === 'full') {
      ...
      strategicRiskPanel.setLocationClickHandler((lat, lon) => {
        this.ctx.map?.setCenter(51, 10, 4);
        console.log("panel-layout setzt center", lat, lon);
      });
      ...
      displacementPanel.setCountryClickHandler((_lat, _lon) => {
        this.ctx.map?.setCenter(51, 10, 4);
      });
      this.ctx.panels['displacement'] = displacementPanel;

      const climatePanel = new ClimateAnomalyPanel();
      climatePanel.setZoneClickHandler((_lat, _lon) => {
        this.ctx.map?.setCenter(51, 10, 4);
      });
    ...
    if (SITE_VARIANT !== 'happy') {
      ...
      //const cyberIntelPanel = new CyberIntelPanel();
      //this.ctx.panels['cyber-intel'] = cyberIntelPanel;

      //const liveWebcamsPanel = new LiveWebcamsPanel();
      //this.ctx.panels['live-webcams'] = liveWebcamsPanel;
      
      const cyberMapPanel = new ExternalWebPanel();
      this.ctx.panels['cyber-map'] = cyberMapPanel;
      
      const cyberIntelPanel = new CyberIntelPanel(() => this.ctx.allNews);
      this.ctx.panels['cyber-intel'] = cyberIntelPanel;

      const sensorNetworkPanel = new SensorNetworkPanel();
      this.ctx.panels['sensor-network'] = sensorNetworkPanel;
...
const cyberIdx = panelOrder.indexOf('cyber-intel');

if (cyberIdx !== -1) {
  panelOrder.splice(cyberIdx, 1);
}

const afterNews = panelOrder.indexOf('live-news') + 1;
panelOrder.splice(afterNews, 0, 'cyber-intel');
}
  ...
  private applyInitialUrlState(): void {
    ...
    if (!view && lat !== undefined && lon !== undefined) {
