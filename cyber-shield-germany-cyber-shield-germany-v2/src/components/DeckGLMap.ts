...
import { getActiveDemoCyberAttackEvents, type DemoCyberAttack } from '@/services/demo-cyber-attacks';

export type TimeRange = '1h' | '6h' | '24h' | '48h' | '7d' | 'all';
export type DeckMapView = 'germany' |'global' | 'america' | 'mena' | 'eu' | 'asia' | 'latam' | 'africa' | 'oceania';
...
const VIEW_PRESETS: Record<DeckMapView, { longitude: number; latitude: number; zoom: number }> = {
  germany: {longitude: 10.5,latitude: 51.2,zoom: 6 },
...
const getDemoCyberColor = (severity: number, alpha: number): [number, number, number, number] => {
  if (severity > 0.72) return [255, 49, 49, alpha];
  if (severity > 0.42) return [255, 170, 0, alpha];
...
export class DeckGLMap {
  ...
  private demoCyberIntervalId: ReturnType<typeof setInterval> | null = null;
  ...
  private startDemoCyberTimer(): void {
    if (this.demoCyberIntervalId) return;
    this.demoCyberIntervalId = setInterval(() => this.rafUpdateLayers(), 1000);
  }

  private stopDemoCyberTimer(): void {
    if (!this.demoCyberIntervalId) return;
    clearInterval(this.demoCyberIntervalId);
    this.demoCyberIntervalId = null;
  }

  private getDemoCyberPacketPosition(attack: DemoCyberAttack, now = Date.now()): [number, number] {
    const progress = ((now - attack.activatedAt) % 6000) / 6000;
    const eased = 0.5 - Math.cos(progress * Math.PI) / 2;
    const [sourceLon, sourceLat] = attack.sourcePosition;
    const [targetLon, targetLat] = attack.targetPosition;
    return [
      sourceLon + (targetLon - sourceLon) * eased,
  ...
  private buildLayers(): LayersList {
    ...
    // Demo cyber attacks for Germany view. Kept separate from the live IOC layer.
    const now = Date.now();
    const demoCyberAttacks = this.state.view === 'germany' ? getActiveDemoCyberAttackEvents(now) : [];
    if (demoCyberAttacks.length > 0) {
      this.startDemoCyberTimer();
      const pulse = 0.5 + 0.5 * Math.sin(now / 650);
      layers.push(
        new ArcLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-arcs',
          data: demoCyberAttacks,
          getSourcePosition: d => d.sourcePosition,
          getTargetPosition: d => d.targetPosition,
          getSourceColor: d => getDemoCyberColor(d.severity, 80),
          getTargetColor: d => getDemoCyberColor(d.severity, 235),
          getWidth: d => d.severity > 0.72 ? 6 : 4,
          widthMinPixels: 2,
          widthMaxPixels: 8,
          greatCircle: true,
          pickable: true,
        }),
        new ScatterplotLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-packets',
          data: demoCyberAttacks,
          getPosition: d => this.getDemoCyberPacketPosition(d, now),
          getRadius: 8500,
          getFillColor: d => getDemoCyberColor(d.severity, 245),
          radiusMinPixels: 5,
          radiusMaxPixels: 12,
          stroked: true,
        ...
        new ScatterplotLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-source-rings',
          data: demoCyberAttacks,
          getPosition: d => d.sourcePosition,
          getRadius: () => 12000 + pulse * 4500,
          filled: false,
          stroked: true,
          getLineColor: d => getDemoCyberColor(d.severity, 95),
          lineWidthMinPixels: 1,
          radiusMinPixels: 8,
          radiusMaxPixels: 20,
          pickable: false,
        ...
        new ScatterplotLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-sources',
          data: demoCyberAttacks,
          getPosition: d => d.sourcePosition,
          getRadius: 10000,
          getFillColor: d => getDemoCyberColor(d.severity, 150),
          radiusMinPixels: 5,
          radiusMaxPixels: 12,
          stroked: true,
        ...
        new ScatterplotLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-target-rings',
          data: demoCyberAttacks,
          getPosition: d => d.targetPosition,
          getRadius: d => 22000 + Math.max(0, Math.sin((now - d.activatedAt) / 500)) * 9000,
          filled: false,
          stroked: true,
          getLineColor: d => getDemoCyberColor(d.severity, 120),
        ...
        new ScatterplotLayer<DemoCyberAttack>({
          id: 'demo-cyber-attack-targets',
          data: demoCyberAttacks,
          getPosition: d => d.targetPosition,
          getRadius: 14000,
          getFillColor: d => getDemoCyberColor(d.severity, 230),
          radiusMinPixels: 10,
          radiusMaxPixels: 24,
          stroked: true,
  ...
  private getTooltip(info: PickingInfo): { html: string } | null {
    ...
    switch (layerId) {
      case 'demo-cyber-attack-arcs':
      case 'demo-cyber-attack-sources':
      case 'demo-cyber-attack-targets':
        return {
          html: `<div class="deckgl-tooltip"><strong>${text(obj.protocol).toUpperCase()} ${text(obj.event_action)}</strong><br/>${text(obj.sourceName || obj.src_ip)} &rarr; ${text(obj.targetName || obj.target?.sensor_name || '')}<br/>${text(obj.event_family)}</div>`,
        };
      case 'hotspots-layer':
