...
export type MapView = 'germany'|'global' | 'america' | 'mena' | 'eu' | 'asia' | 'latam' | 'africa' | 'oceania';
...
if (!projection) return;
  ...
  console.log("[Resize] New size:", width, height);
  ...
  private setupZoomHandlers(): void {
	...
	const tx = centerOffsetX;
	const ty = centerOffsetY;
...
if (!projection) return;
  ...
  public render(): void {
    ...
    console.log("Layer:", this.dynamicLayerGroup);
  ...
  this.lastView !== this.state.view;
    ...
    if (shouldRenderBase) {
     ...
     //const projection = this.projection;
     //if (!projection) return;
     const basePath = d3.geoPath().projection(this.projection);
      //const basePath = d3.geoPath().projection(projection);
...
// === DEMO CYBER ATTACKS (Germany SVG Mode) ===
if (this.state.view === 'germany') {



  const svg = this.dynamicLayerGroup?.node();
  if (!svg) return;

  // 🔥 WICHTIG: vorher alles löschen (nur EINMAL!)
  //this.dynamicLayerGroup?.selectAll("*").remove();

  const germany = projection([10.45, 51.16]);
  if (!germany) return;
  ...
  demoPoints.forEach(point => {
    ...
    const severity = (['low','medium','high'] as const)[Math.floor(Math.random()*3)] ?? 'low';
	...
	const size = Math.max(0.5, 2 / zoom);

	circle.setAttribute('r', String(size));
    circle.setAttribute('fill', severityColors[severity]);
    ...
    const midX = (pos[0] + germany[0]) / 2;
    const midY = (pos[1] + germany[1]) / 2 - 60;

    const d = `M ${pos[0]} ${pos[1]} Q ${midX} ${midY} ${germany[0]} ${germany[1]}`;
    

    path.setAttribute('d', d);
    path.setAttribute('stroke', severityColors[severity]);
...
private getProjection(width: number, height: number): d3.GeoProjection {
  ...
  const scale = (width / (2 * Math.PI)) * zoom;
...
private applyTransform(): void {
 this.wrapper.style.transform = `scale(${this.state.zoom})`;
}
...
private hasCentered = false;
  ...
  console.log("FIT COUNTRY RUNNING");
...
if (!projection) return;
    ...
    if (!topLeft || !bottomRight) {
      this.state.zoom = 2;
if (!this.hasCentered) {
this.setCenter(51, 10); // Deutschland Mitte
...
this.setCenter(51, 10); // Deutschland Mitte
  ...
  console.log("CENTER TARGET:", midLat, midLon);
});
  }
